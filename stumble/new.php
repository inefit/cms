...
function submit_form(data, form, json, options){	
	...
	if (data == true){
		...
		setTimeout("load_page('?act=stumble','')",1005);
	}else {
...
<form mehod="POST" action="index.php" id='ajax_form'>
	...
	<label>URL </label> : &nbsp;
	<input id="stumble_url"  name="data[stumble_url]" type="text" class="validate[required,custom[url]]"  value="<?=$data['stumble_url']?>" size="50"/> <br>
	<label>Star </label> : &nbsp;
	<input  name="data[stumble_seed]" type="text" class="nomer"  value="<?=$data['stumble_seed']?>" size="30"/> <br>
	<br>
