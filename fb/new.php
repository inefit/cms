...
function submit_form(data, form, json, options){	
	...
	if (data == true){
		...
		setTimeout("load_page('?act=fb','')",1005);
	}else {
...
<form mehod="POST" action="index.php" id='ajax_form'>
	...
	<label>URL </label> : &nbsp;
	<input id="fb_url"  name="data[fb_url]" type="text" value="<?=$data['fb_url']?>" size="50"/> <br>
	<label>Star </label> : &nbsp;
	<input  name="data[fb_seed]" type="text" class="nomer"  value="<?=$data['fb_seed']?>" size="30"/> <br>
	<br>
