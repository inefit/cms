...
function submit_form(data, form, json, options){	
	...
	if (data == true){
		...
		setTimeout("load_page('?act=gplus','')",1005);
	}else {
...
<form mehod="POST" action="index.php" id='ajax_form'>
	...
	<label>URL </label> : &nbsp;
	<input id="gplus_url"  name="data[gplus_url]" type="text" class="validate[required,custom[url]]"  value="<?=$data['gplus_url']?>" size="50"/> <br>
	<label>Star </label> : &nbsp;
	<input  name="data[gplus_seed]" type="text" class="nomer"  value="<?=$data['gplus_seed']?>" size="30"/> <br>
	<br>
