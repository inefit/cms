...
function submit_form(data, form, json, options){	
	...
	if (data == true){
		...
		setTimeout("load_page('?act=digg','')",1005);
	}else {
...
<form mehod="POST" action="index.php" id='ajax_form'>
	...
	<label>Digg User </label> : &nbsp;
	<input id="digg_user"  name="data[digg_user]" type="text" class="validate[required,custom[url]]"  value="<?=$data['digg_user']?>" size="50"/> <br>
	<label>Star </label> : &nbsp;
	<input  name="data[digg_seed]" type="text" class="nomer"  value="<?=$data['digg_seed']?>" size="30"/> <br>
	<br>
